...
/** Active prompt version — severity "consistent" uses nullable excerpt. */
export const INCIDENT_CLASSIFICATION_V3_PROMPT_VERSION = "incident-classification-v3";
...
- categoryEvidenceExcerpt MUST be copied verbatim from the incident description (exact substring).
...
- If uncertain about category, use "uncertain" in the relevant assessment field.
...
## Severity evidence rules (critical)
- If severityAssessment is "possibly_understated", "possibly_overstated", or "uncertain": severityEvidenceExcerpt MUST be a non-null verbatim supporting excerpt copied from the description that relates to harm seriousness or operational impact.
- If severityAssessment is "consistent": severityEvidenceExcerpt MUST be null.
- For severity "consistent", do NOT invent text such as "no serious injury", "severity appears appropriate", "consistent with Low", or any other phrase that is not in the description.
- Consistency with recorded severity is represented by assessment "consistent" plus a null excerpt — never by inventing evidence of consistency.
...
For severity "consistent", explain cautiously that no clear inconsistency signal was identified in the supplied description and human review is still recommended.
